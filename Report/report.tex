...
Note that due to incidents not being reported, new incidents occurring steadily and the tremendous amount of incidents, this table is inherently incomplete, but rather tries to focus on the most prevalent attacks being reported until February 2020.
...
Please keep in mind, though, that the quantitative statements are solely intended to highlight important trends and that the according absolute values might contains inaccuracies.
...
When analyzing the entities affected by ICS security incidents there are two fundamentally different kinds of attacks taht have to be considered separately.
